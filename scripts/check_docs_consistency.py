...
The two sources of truth for the project's AccessibilityID constants are:
  ...
  2. `docs/ax-id-reference.md`
     ...
     with a known AX-ID prefix (btn-, field-, lbl-, slider-, scroll-).
...
_KNOWN_PREFIXES = ("btn-", "field-", "lbl-", "slider-", "scroll-")
...
def parse_md_reference(md_text: str) -> set[str]:
    ...
    Matches rows whose first cell starts with one of the 5 known AX-ID prefixes:
